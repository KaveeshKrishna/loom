...
export function SetupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Account created — sign the new owner in immediately.
      const signInRes = await signIn.email({ email, password });
      if (signInRes.error) {
        // Account exists, just route to login instead of failing silently.
        router.push("/login");
        return;
      }
      router.push("/files");
    ...
    <div className="min-h-svh bg-[hsl(var(--background))] flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <p className="text-center text-xs text-[hsl(var(--muted-foreground))] mt-6">
          This account will have full administrative access to Loom.
        </p>
      </div>
    </div>
  );
